...
int agcProcess(int16_t *buffer, uint32_t sampleRate, size_t samplesCount, int16_t agcMode) {
    ...
    awi_ai_vad_cfg_init(&vad_cfg);
    awi_ai_vad_t vad ;
    awi_ai_vad_init(&vad, &vad_cfg);
    agcConfig.compressionGaindB = 40; // default 9 dB
    ...
    int16_t ai_vad_outBuffer[samples];
    int16_t *out16 = out_buffer;
    int16_t *aiVad_out = ai_vad_outBuffer;
    uint8_t saturationWarning = 1;                 //是否有溢出发生，增益放大以后的最大值超过了65536
    int16_t echo = 0;                                 //增益放大是否考虑回声影响
    for (int i = 0; i < nTotal; i++) {
        inMicLevel = 0;
        int nAgcRet;
        nAgcRet = WebRtcAgc_Process(agcInst, &vad, (const int16_t *const *) &input, samples,
                                    (int16_t *const *) &out16, aiVad_out, inMicLevel, &outMicLevel, echo,
                                    &saturationWarning);
        if(nAgcRet == 0){
            memcpy(input, aiVad_out, samples * sizeof(int16_t));
            input += samples;
        }
        else if (nAgcRet ==-1){
            input += samples;
    ...
    if (remainedSamples > 0) {
        ...
        inMicLevel = 0;
        int nAgcRet = WebRtcAgc_Process(agcInst,&vad, (const int16_t *const *) &input, samples,
                                        (int16_t *const *) &out16,aiVad_out, inMicLevel, &outMicLevel, echo,
                                        &saturationWarning);
        ...
        memcpy(&input[samples-remainedSamples], &aiVad_out[samples-remainedSamples], remainedSamples * sizeof(int16_t));
        input += samples;
    }

    WebRtcAgc_Free(agcInst);
    awi_ai_vad_deinit(&vad);
    return 1;
}
...
int main(int argc, char *argv[]) {
    ...
    sprintf(out_file, "%s%s%s_nnout%s", drive, dir, fname, ext);
